...
namespace OpenshiftOPSCenter.App.Services
{
    public class OpenShiftService
    {
        ...
        private void InitializeOcPath()
        {
            try
            {
                var ocFolder = _settingsService.GetOcToolFolderAsync().GetAwaiter().GetResult();
                if (!string.IsNullOrWhiteSpace(ocFolder))
                {
                    var candidate = Path.Combine(ocFolder, "oc.exe");
                    _ocPath = candidate;
                }
                else
                {
                ...
                _loggingService.LogInfo($"OC-Tool Pfad verwendet: {_ocPath}");
            }
            catch (Exception ex)
            {
                _ocPath = "oc";
                _loggingService.LogWarning($"Konnte OC-Tool-Pfad nicht initialisieren, verwende 'oc' aus PATH. Grund: {ex.Message}");
            }
        }

        public void RefreshOcPath()
        {
            InitializeOcPath();
        }
        ...
        public async Task<(bool success, string message)> LoginToCluster(string clusterUrl, string username, string password)
        {
            try
            {
                ...
                // Wenn ein absoluter Pfad/Dateiname verwendet wird, prüfen wir auf Existenz, ansonsten vertrauen wir PATH
                bool looksLikeExplicitPath = _ocPath.Contains(Path.DirectorySeparatorChar) || _ocPath.Contains(Path.AltDirectorySeparatorChar);
                if (looksLikeExplicitPath && !File.Exists(_ocPath))
                {
        ...
        // Method to get all unbound PVs related to current project
        public async Task<(List<PersistentVolume> pvs, string error)> GetUnboundPersistentVolumes()
        {
            var pvs = new List<PersistentVolume>();
            try
            {
                ...
                _loggingService.LogInfo($"Lade ungebundene PVs für Projekt {currentProject}...");
                ...
                using (var process = new Process { StartInfo = startInfo })
                {
                    ...
                    if (process.ExitCode == 0)
                    {
                        ...
                        if (pvList != null && pvList.items != null)
                        {
                            foreach (var item in pvList.items)
                            {
                                // Nur PVs hinzufügen, die nicht den Status "Bound" haben und mit dem aktuellen Projekt in Zusammenhang stehen
                                bool isRelatedToCurrentProject = false;
                                
                                // Prüfen, ob der Name des PV das aktuelle Projekt enthält oder ob claimRef auf das aktuelle Projekt verweist
                                if (item.metadata.name.Contains(currentProject, StringComparison.OrdinalIgnoreCase))
                                {
                                    isRelatedToCurrentProject = true;
                                }
                                else if (item.spec.claimRef != null && 
                                         item.spec.claimRef.namespaceName == currentProject)
                                {
                                    isRelatedToCurrentProject = true;
                                }
                                
                                if (isRelatedToCurrentProject && item.status.phase != "Bound")
                                {
                                    var pv = new PersistentVolume
                                    {
                                        Name = item.metadata.name,
                        ...
                        _loggingService.LogInfo($"Ungebundene PVs erfolgreich geladen: {pvs.Count}");
                        return (pvs, null);
                    }
                    else
                    {
                        var errorMessage = $"Fehler beim Laden der ungebundenen PVs. Exit Code: {process.ExitCode}. Fehler: {error}";
                        _loggingService.LogError(errorMessage);
                        return (pvs, errorMessage);
                    }
                }
            ...
            {
                var errorMessage = $"Fehler beim Laden der ungebundenen PVs: {ex.Message}";
                _loggingService.LogError(errorMessage, ex);
                return (pvs, errorMessage);
            }
        }
        ...
        public async Task<string> GetHostDeploymentYamlAsync(string projectName)
        {
            try
            {
                // Überprüfen, ob Projektname leer ist
                if (string.IsNullOrEmpty(projectName))
                {
                    throw new ArgumentException("Das Projekt wurde nicht angegeben. Bitte wählen Sie zuerst ein Projekt aus.");
                }
                ...
                using (var process = new Process { StartInfo = startInfo })
                {
                    ...
                    else
                    {
                        if (error.Contains("not found"))
                        {
                            ...
                            try
                            {
                                var deployments = await GetDeploymentsMinimal(projectName);
                                if (deployments.deployments.Count > 0)
                                {
                                    // Verwende das erste verfügbare Deployment
                                    var firstDeployment = deployments.deployments[0].Name;
                                    _loggingService.LogInfo($"Kein 'host' Deployment gefunden. Verwende stattdessen: {firstDeployment}");
                                    
                                    // Versuche erneut mit dem ersten Deployment
                                    var altStartInfo = new ProcessStartInfo
                                    {
                                        FileName = _ocPath,
                                        Arguments = $"get deployment {firstDeployment} -n \"{projectName}\" -o yaml",
                                        UseShellExecute = false,
                                        RedirectStandardOutput = true,
                                        RedirectStandardError = true,
                                        CreateNoWindow = true
                                    ...
                                    using (var altProcess = new Process { StartInfo = altStartInfo })
                                    {
                                        var altOutputBuilder = new StringBuilder();
                                        var altErrorBuilder = new StringBuilder();

                                        altProcess.OutputDataReceived += (sender, e) => 
                                        {
                                            if (e.Data != null)
                                            {
                                                altOutputBuilder.AppendLine(e.Data);
                                            }
                                        };

                                        altProcess.ErrorDataReceived += (sender, e) => 
                                        {
                                            if (e.Data != null)
                                            {
                                                altErrorBuilder.AppendLine(e.Data);
                                            }
                                        };
                                        ...
                                        var altOutput = altOutputBuilder.ToString();
                                        var altError = altErrorBuilder.ToString();

                                        if (altProcess.ExitCode == 0)
                                        {
                                            _loggingService.LogInfo($"Alternatives Deployment YAML erfolgreich geladen: {firstDeployment}");
                                            return altOutput;
                                        }
                                    }
                                }
                            ...
                            {
                                _loggingService.LogWarning($"Fehler beim Abrufen alternativer Deployments: {deploymentEx.Message}");
                            }
                            
                            throw new Exception($"Im Projekt '{projectName}' konnte kein Deployment mit dem Namen 'host' gefunden werden.");
                        }
                        else
                        {
        ...
        {
            return await CreateResourceFromYamlAsync(yamlContent, false);
        }

        // Methode zum Erstellen einer Ressource aus YAML mit Option zum Aktualisieren
        public async Task<bool> CreateResourceFromYamlAsync(string yamlContent, bool update)
        {
            try
            {
                _loggingService.LogInfo(update ? "Aktualisiere Ressource aus YAML" : "Erstelle Ressource aus YAML");
                
                // Erstelle eine temporäre YAML-Datei
                string tempFile = Path.GetTempFileName() + ".yaml";
                await File.WriteAllTextAsync(tempFile, yamlContent);
                
                // Verwende create oder apply je nach update-Parameter
                string command = update ? "apply" : "create";
                
                var startInfo = new ProcessStartInfo
                {
                    FileName = _ocPath,
                    Arguments = $"{command} -f \"{tempFile}\"",
                    UseShellExecute = false,
                    RedirectStandardOutput = true,
                    RedirectStandardError = true,
                    CreateNoWindow = true
                ...
                using (var process = new Process { StartInfo = startInfo })
                {
                    ...
                    if (process.ExitCode == 0)
                    {
                        string action = update ? "aktualisiert" : "erstellt";
                        _loggingService.LogInfo($"Ressource erfolgreich {action}: {output}");
                        return true;
                    }
                    else
                    {
                        string action = update ? "Aktualisieren" : "Erstellen";
                        var errorMessage = $"Fehler beim {action} der Ressource. Exit Code: {process.ExitCode}. Fehler: {error}";
                        _loggingService.LogError(errorMessage);
                        return false;
                    }
                }
            ...
            {
                string action = update ? "Aktualisieren" : "Erstellen";
                var errorMessage = $"Fehler beim {action} der Ressource: {ex.Message}";
                _loggingService.LogError(errorMessage, ex);
                return false;
            }
        }
        ...
        // Hinzufügen der Methode für das Laden des Deployment YAMLs
        public async Task<string> GetDeploymentYamlAsync(string projectName, string deploymentName)
        {
            try
            {
                _loggingService.LogInfo($"Lade YAML für Deployment {deploymentName} in Projekt {projectName}");
                
                var startInfo = new ProcessStartInfo
                {
                    FileName = _ocPath,
                    Arguments = $"get deployment {deploymentName} -n {projectName} -o yaml",
                    UseShellExecute = false,
                    RedirectStandardOutput = true,
                    RedirectStandardError = true,
                    CreateNoWindow = true
                ...
                using (var process = new Process { StartInfo = startInfo })
                {
                    ...
                    if (process.ExitCode == 0)
                    {
                        _loggingService.LogInfo($"YAML für Deployment {deploymentName} erfolgreich geladen");
                        return output;
                    }
                    else
                    {
                        // Prüfen, ob der Fehler "not found" enthält
                        if (error.Contains("NotFound") || error.Contains("not found"))
                        {
                            _loggingService.LogWarning($"Deployment {deploymentName} wurde in Projekt {projectName} nicht gefunden.");
                            return string.Empty;
                        }
                        
                        var errorMessage = $"Fehler beim Laden des YAMLs für Deployment {deploymentName}: {error}";
                        _loggingService.LogError(errorMessage);
                        return string.Empty;
                    }
                }
            ...
            {
                var errorMessage = $"Fehler beim Laden des YAMLs für Deployment {deploymentName}: {ex.Message}";
                _loggingService.LogError(errorMessage, ex);
                return string.Empty;
            }
        }
